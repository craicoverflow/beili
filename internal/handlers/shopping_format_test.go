...
func TestFormatShoppingItem(t *testing.T) {
	tests := []struct {
		...
		// no stored name -> fallback to regex parsing (unchanged behaviour)
		...
		// stored name present -> AI-cleaned name wins, amount still parsed from raw
		{"stored name strips prep note", "240 ml cherry tomatoes, halved", "Cherry tomatoes", "Cherry tomatoes (240ml)"},
		{"stored name strips prep note, bare count", "2 onions, diced", "Onions", "Onions (2)"},
		{"stored name, no parseable amount", "salt to taste", "Salt", "Salt"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := formatShoppingItem(tt.in, tt.storedName); got != tt.want {
				t.Errorf("formatShoppingItem(%q, %q) = %q, want %q", tt.in, tt.storedName, got, tt.want)
			}
		})
	}
}

...
func (s *MealStore) Create(ctx context.Context, meal *models.Meal, sources []models.Source) error {
	...
	_, err = tx.ExecContext(ctx, `
		...
		meal.PrepTime, meal.CookTime, meal.Servings, meal.Ingredients, meal.Rating, meal.Notes,
...
func (s *MealStore) Update(ctx context.Context, meal *models.Meal, sources []models.Source) error {
	...
	_, err = tx.ExecContext(ctx, `
		...
		meal.Rating, meal.Notes, meal.UpdatedAt,
		meal.ID,
	)
	if err != nil {
		tx.Rollback()
...
func scanMeals(rows *sql.Rows) ([]models.Meal, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&m.PrepTime, &m.CookTime, &m.Servings, &m.Ingredients, &m.Rating, &m.Notes,
